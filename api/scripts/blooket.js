(() => {
    ...
    // Simple guaranteed working menu
    let gui = null;
    ...
    function createSimpleMenu() {
        ...
        header.innerHTML = '🎮  -  to hide the menu press ctrl e';
        ...
        modeSelector.innerHTML = `
            <select id="gameMode" style="background: #333; color: white; border: 1px solid red; padding: 3px; width: 100%;">
                <option value="global">🌍 Global</option>
                <option value="gold">⚡ Gold Quest</option>
                <option value="hack">💻 Crypto Hack</option>
                <option value="fish">🎣 Fishing Frenzy</option>
                <option value="pirate">🏴 Pirate's Voyage</option>
                <option value="defense2">🏯 Tower Defense 2</option>
                ...
                <option value="brawl">⚔️ Brawl</option>
                <option value="dino">🦕 Dino Dig</option>
        ...
        // Helper function to get React state - exact from original
        ...
        // COMPLETE ORIGINAL CHEATS FROM inspect console.txt - EXACT COPY
        ...
        const Cheats = {
            ...
            cafe: [
                {
                    name: "Max Items",
                    description: "Maxes out all items",
                    run: function () {
                        let stateNode = getStateNode();
                        stateNode.state.items.forEach((item) => {
                            for (let i = 0; i < 5; i++) stateNode.state.upgrades[item]++;
                        });
                    },
                },
                {
                    name: "Set Customers",
                    description: "Sets amount of customers",
                    inputs: [
                        {
                            name: "Customers",
                            type: "number",
                    ...
                    run: function (customers) {
                        getStateNode().setState({ customers });
                    },
                },
                {
                    name: "Set Money",
            ...
            factory: [
                {
                    name: "Max Blooks",
                    description: "Maxes out all blooks",
                    run: function () {
                        let stateNode = getStateNode();
                        stateNode.state.blooks.forEach((blook) => {
                            stateNode.state.blooks[blook.id] = 100;
                        });
                    },
                },
                {
            ...
            racing: [
                {
                    name: "Instant Win",
                    description: "Instantly win the race",
                    run: function () {
                        let stateNode = getStateNode();
                        stateNode.state.progress = 100;
                    },
                },
                {
                    name: "Set Speed",
            ...
            rush: [
                {
                    name: "Max Points",
                    description: "Maxes out points",
                    run: function () {
                        getStateNode().setState({ points: 1000 });
                    },
                },
                {
                    name: "Set Points",
            ...
            kingdom: [
                {
                    name: "Max Resources",
                    description: "Maxes out all resources",
                    run: function () {
                        let stateNode = getStateNode();
                        stateNode.state.resources.forEach((resource) => {
                            stateNode.state.resources[resource.id] = 1000;
                        });
                    },
                },
                {
            ...
            santa: [
                {
                    name: "Max Gifts",
                    description: "Maxes out all gifts",
                    run: function () {
                        let stateNode = getStateNode();
                        stateNode.state.gifts.forEach((gift) => {
                            stateNode.state.gifts[gift.id] = 100;
                        });
                    },
                },
                {
                    name: "Set Nice",
                    description: "Sets nice level",
                    inputs: [
                        {
                            name: "Nice",
                            type: "number",
            ...
            flappy: [
                {
                    ...
                    run: function () {
                        if (!this.enabled) {
                            this.enabled = true;
                            this.data = setInterval(() => {
                                getStateNode().setState({ invincible: true });
                            }, 50);
                        } else {
                            this.enabled = false;
                            clearInterval(this.data);
        ...
        // Function to create cheat item with button
        function createCheatItem(cheat, index) {
            ...
            name.textContent = `${index + 1}. ${cheat.name}`;
            name.style.flexGrow = '1';
            
            // Create input field for cheats that need parameters
            let input = null;
            if (cheat.inputs && cheat.inputs.length > 0) {
                input = document.createElement('input');
                input.type = cheat.inputs[0].type === 'number' ? 'number' : 'text';
                input.placeholder = cheat.inputs[0].name;
                ...
                // Set default values
                if (cheat.inputs[0].type === 'number') {
                    input.value = cheat.name.includes('Gold') ? '1000' : '100';
                }
            }
            
            const button = document.createElement('button');
            button.style.background = cheat.type === 'toggle' && cheat.enabled ? 'green' : 'red';
            button.style.color = 'white';
            button.style.border = 'none';
            button.style.padding = '2px 6px';
            button.style.fontSize = '10px';
            button.style.cursor = 'pointer';
            button.textContent = cheat.type === 'toggle' ? (cheat.enabled ? 'DISABLE' : 'ENABLE') : 'EXECUTE';
            
            button.onclick = () => {
                if (cheat.type === 'toggle') {
                    // Call the cheat run function first (this handles the toggle logic)
                    if (input) {
                        const value = input.type === 'number' ? parseFloat(input.value) || 1000 : input.value;
                        cheat.run(value);
                    } else {
                        cheat.run();
                    }
                    // Update button state after cheat runs
                    button.textContent = cheat.enabled ? 'DISABLE' : 'ENABLE';
                    button.style.background = cheat.enabled ? 'green' : 'red';
                    console.log(`${cheat.name} ${cheat.enabled ? 'enabled' : 'disabled'}`);
                } else {
                    // For execute buttons, call run with input value if needed
                    if (input) {
                        const value = input.type === 'number' ? parseFloat(input.value) || 1000 : input.value;
                        cheat.run(value);
                    } else {
                    ...
                    console.log(`${cheat.name} executed`);
                }
            };
        ...
        cheats.id = 'cheatsList';
        
        // Function to update cheats display
        function updateCheatsDisplay(mode) {
            cheats.innerHTML = '';
            const cheatsList = Cheats[mode] || Cheats.global;
            cheatsList.forEach((cheat, index) => {
                cheats.appendChild(createCheatItem(cheat, index));
            });
        }
        
        // Update cheats when mode changes
        modeSelector.querySelector('#gameMode').addEventListener('change', (e) => {
            updateCheatsDisplay(e.target.value);
        });
        
        // Initial display
        updateCheatsDisplay('global');
        ...
        gui.appendChild(modeSelector);
        gui.appendChild(cheats);
    ...
    setTimeout(() => {
        createSimpleMenu();
        console.log('🎮  LOADED! Menu should be visible now.');
    }, 1000);
    ...
    console.log('🎮  Script Loaded - Menu will appear in 1 second');
